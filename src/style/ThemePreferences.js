...
export default class ThemePreferences {
  ...
  async migrateThemePrefForModes() {
    ...
    const systemThemeId = await UserPrefsManager.get().getPref({ key: savedSystemThemeIdKey });
    const savedTheme = await UserPrefsManager.get().getPref({ key: savedThemeKey });

    let themeData;
    if(savedTheme) {
      themeData = savedTheme;
    } else if(systemThemeId) {
      const systemTheme = _.find(StyleKit.get().systemThemes, { uuid: systemThemeId });
    ...
    await UserPrefsManager.get().setPref({ key: THEME_PREFERENCES_KEY, value: this.data });
  ...
  async loadFromStorage() {
    this.data = await UserPrefsManager.get()
      .getPref({ key: THEME_PREFERENCES_KEY });
  ...
  saveToStorage() {
    ...
    this.saveAction = setTimeout(async () => {
      await UserPrefsManager.get()
        .setPref({ key: THEME_PREFERENCES_KEY, value: this.data });
    }, 250);
  }
}
